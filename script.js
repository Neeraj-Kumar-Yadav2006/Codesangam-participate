  ...
  for (let i = 0; i < n - 1; i++) {
    for (let j = 0; j < n - i - 1; j++) {
      ...
      if (arr[j] > arr[j + 1]) {
        ...
        bars[j + 1].querySelector(".bar-value").innerText = arr[j + 1];
...
async function insertionSort() {
  ...
  for (let i = 1; i < n; i++) {
    ...
    bars[j + 1].querySelector(".bar-value").innerText = key;
...
async function selectionSort() {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    if (minIndex !== i) {
      ...
      bars[minIndex].querySelector(".bar-value").innerText = arr[minIndex];
...
async function partition(start, end, bars) {
  ...
  bars[end].querySelector(".bar-value").innerText = arr[end];
...
document.getElementById("sort").addEventListener("click", async () => {
  ...
  else if (algorithm == "merge") await mergeSort(0, arr.length - 1);
  else if (algorithm == "quick") await quickSort(0, arr.length - 1);
});
